...
void DFS_Visit(Graph G, List S, int u, int *time){
   ...
   G->IsDiscover[u] = *time;
   G->IsColor[u] = 5;
   if(!isEmpty(G->IsAdjList[u])){ // Safe gaurd for moveFront segfault.
      moveFront(G->IsAdjList[u]);
      while(index(G->IsAdjList[u]) != -1){ // Process adjList.
         int v = get(G->IsAdjList[u]);
         if(G->IsColor[v] == 4){
            G->IsParent[v] = u;
            DFS_Visit(G, S, v, time);
         }
         moveNext(G->IsAdjList[u]); 
      }
   }
   G->IsColor[u] = 6;
   (*time)++;
...
void DFS(Graph G, List S){
   ...
   for(int i=1; i<(G->orderNumV + 1); i++){ //If you call DFS more than
	  ...
	  G->IsFinish[i] = UNDEF; // The first runtime is the trivial case
   ...
   //fprintf(stdout, "%d\n", get(S));
   while(index(S) != -1){ // first controls the list. index(S) = numV
      int u = front(S);
	  //fprintf(stdout, "%d %d %d\n", i, index(S), u); //For debugging...
	  //fprintf(stdout, "%d %d\n", i, get(S));
	  if(G->IsColor[u] == 4){
...
Graph transpose (Graph G){
   ...
   for(int i = 1; i<(getOrder(G)+1); i++){ // Loop through each adjList[i] in G
      ...
      while(index(G->IsAdjList[i]) != -1){ // Loop through each List element
...
Graph copyGraph(Graph G){
   ...
   for(int i = 1; i<(getOrder(G)+1); i++){
      ...
      //fprintf(stdout, "%d\n", i); //for debugging...
	     moveFront(G->IsAdjList[i]);
         while(index(G->IsAdjList[i]) != -1){
	        addArc(copy, i, get(G->IsAdjList[i]));
			moveNext(G->IsAdjList[i]);
		 }
      }
   }
